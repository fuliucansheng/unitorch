...
def _remap_key(hf_key: str) -> Optional[str]:
    ...
    k = re.sub(
        ...
        "mlp.shared_expert.linear_fc1_gate.weight",
        k,
    )
    k = re.sub(
    ...
    k = re.sub(
        ...
        lambda m: f"mlp.experts.local_experts.{m.group(1)}.linear_fc1_gate.weight",
        k,
    )
    k = re.sub(
        r"mlp\.experts\.(\d+)\.up_proj\.weight",
        lambda m: f"mlp.experts.local_experts.{m.group(1)}.linear_fc1.weight",
        k,
    )
    k = re.sub(
        r"mlp\.experts\.(\d+)\.down_proj\.weight",
        lambda m: f"mlp.experts.local_experts.{m.group(1)}.linear_fc2.weight",
        k,
    )
...
def convert(
    ...
    print(
        f"Config: layers={num_layers}, first_k_dense={first_k_dense}, "
    ...
    for pp_rank in range(pp_size):
        ...
        for tp_rank in range(tp_size):
            ...
            print(
                f"  saved tp={tp_rank} pp={pp_rank} ep={ep_rank} "
                f"→ {out_path}  ({n_params/1e9:.2f}B params)"
            )

    print(f"\nDone. {total} shard files written to {out_dir}")
    print(
        "Load each shard by setting pretrained_weight_path to the corresponding file."
    )
