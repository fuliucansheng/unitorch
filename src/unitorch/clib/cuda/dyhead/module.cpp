...
int deform_conv_backward_parameters_cuda(at::Tensor input,
                                         at::Tensor offset,
                                         at::Tensor gradOutput,
...
at::Tensor sigmoid_focal_loss_forward_cuda(const at::Tensor &logits,
                                           const at::Tensor &targets,
                                           const int num_classes,
                                           const float gamma,
                                           const float alpha);

at::Tensor sigmoid_focal_loss_backward_cuda(const at::Tensor &logits,
                                            const at::Tensor &targets,
                        ...
                        int im2col_step)
{
    ...
    {
        return deform_conv_forward_cuda(input, weight, offset, output, columns, ones, kW, kH, dW,
                               ...
                               int im2col_step)
{
    if (input.device().is_cuda())
    {
        return deform_conv_backward_input_cuda(
            input, offset, gradOutput, gradInput, gradOffset, weight, columns, kW, kH, dW, dH, padW,
            padH, dilationW, dilationH, group, deformable_group, im2col_step);
    }
    AT_ERROR("Not implemented on the CPU");
}
                                    ...
                                    int im2col_step)
{
    if (input.device().is_cuda())
    {
        return deform_conv_backward_parameters_cuda(
            input, offset, gradOutput, gradWeight, columns, ones, kW, kH, dW, dH, padW, padH,
            dilationW, dilationH, group, deformable_group, scale, im2col_step);
    }
    AT_ERROR("Not implemented on the CPU");
}
                                   ...
                                   const bool with_bias)
{
    if (input.device().is_cuda())
    {
        return modulated_deform_conv_cuda_forward(
            input, weight, bias, ones, offset, mask, output, columns, kernel_h, kernel_w, stride_h,
            stride_w, pad_h, pad_w, dilation_h, dilation_w, group, deformable_group, with_bias);
    }
    AT_ERROR("Not implemented on the CPU");
}
                                    ...
                                    const bool with_bias)
{
    if (input.device().is_cuda())
    {
        return modulated_deform_conv_cuda_backward(
            input, weight, bias, ones, offset, mask, columns, grad_input, grad_weight, grad_bias,
            grad_offset, grad_mask, grad_output, kernel_h, kernel_w, stride_h, stride_w, pad_h,
            pad_w, dilation_h, dilation_w, group, deformable_group, with_bias);
    }
    AT_ERROR("Not implemented on the CPU");
}

at::Tensor sigmoid_focal_loss_forward(const at::Tensor &logits,
                                      const at::Tensor &targets,
                                      const int num_classes,
                                      const float gamma,
                                      const float alpha)
{
    ...
    {
        return sigmoid_focal_loss_forward_cuda(logits, targets, num_classes, gamma, alpha);
    }
    AT_ERROR("Not implemented on the CPU");
}

at::Tensor sigmoid_focal_loss_backward(const at::Tensor &logits,
                                       const at::Tensor &targets,
                                       const at::Tensor &d_losses,
                                       const int num_classes,
                                       const float gamma,
                                       const float alpha)
{
    if (logits.device().is_cuda())
    {
        return sigmoid_focal_loss_backward_cuda(logits, targets, d_losses, num_classes, gamma,
                                                alpha);
    }
    AT_ERROR("Not implemented on the CPU");
}
