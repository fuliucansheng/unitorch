...
class GenericMultiControlNetLoraModel(GenericPeftModel, QuantizationMixin):
...
class MultiControlNetLoraForText2ImageGeneration(GenericMultiControlNetLoraModel):
    def __init__(
        self,
        config_path: str,
        text_config_path: str,
    ...
    def generate(
        ...
        images = self.pipeline(
            image=condition_pixel_values.transpose(0, 1),
...
class MultiControlNetLoraForImage2ImageGeneration(GenericMultiControlNetLoraModel):
    def __init__(
        self,
        config_path: str,
        text_config_path: str,
    ...
    def generate(
        ...
        images = self.pipeline(
            image=pixel_values,
            control_image=condition_pixel_values.transpose(0, 1),
            prompt_embeds=prompt_embeds,
            negative_prompt_embeds=negative_prompt_embeds,
            generator=torch.Generator(device=self.pipeline.device).manual_seed(
                self.seed
...
class MultiControlNetLoraForImageInpainting(GenericMultiControlNetLoraModel):
    def __init__(
        self,
        config_path: str,
        text_config_path: str,
