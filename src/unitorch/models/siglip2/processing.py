...
from transformers import Siglip2ImageProcessor, SiglipTokenizer
...
class Siglip2Processor(HfTextClassificationProcessor):
    def __init__(
        self,
        ...
        """
        Initializes the Siglip2Processor.
        ...
        if vision_config_path is not None:
            self.vision_processor = Siglip2ImageProcessor.from_json_file(
                vision_config_path
            )
        else:
            self.vision_processor = Siglip2ImageProcessor.from_pretrained(
                "google/siglip2-base-patch16-224"
            )
    ...
    def image_classification(
        ...
        Returns:
            GenericOutputs: An object containing the processed inputs.
        """
        pixel_outputs = self.vision_processor.preprocess(image, return_tensors="pt")

        return GenericOutputs(
            pixel_values=pixel_outputs.pixel_values[0],
            pixel_masks=pixel_outputs.pixel_attention_mask[0],
            spatial_shapes=pixel_outputs.spatial_shapes[0],
        )
