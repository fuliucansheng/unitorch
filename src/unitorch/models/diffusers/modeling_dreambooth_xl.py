...
class DreamboothXLForText2ImageGeneration(GenericModel, QuantizationMixin):
    ...
    def forward(
        ...
        if class_pixel_values is None:
            ...
            mix_prompt_embeds = prompt_embeds
            mix_pooled_prompt_embeds = pooled_prompt_embeds
            mix_add_time_ids = add_time_ids
        else:
            ...
            class_prompt_outputs = self.text(
                class_input_ids,
                class_attention_mask,
                output_hidden_states=True,
            )
            class_prompt_embeds = class_prompt_outputs.hidden_states[-2]
            class_prompt2_outputs = self.text2(
                class_input2_ids,
                class_attention2_mask,
                output_hidden_states=True,
            )
            class_prompt2_embeds = class_prompt2_outputs.hidden_states[-2]
            class_prompt_embeds = torch.concat(
                [class_prompt_embeds, class_prompt2_embeds], dim=-1
            )
            class_pooled_prompt_embeds = class_prompt2_outputs[0]
            mix_prompt_embeds = torch.cat([prompt_embeds, class_prompt_embeds], dim=0)
            mix_pooled_prompt_embeds = torch.cat(
                [pooled_prompt_embeds, class_pooled_prompt_embeds], dim=0
            )
            mix_add_time_ids = torch.cat([add_time_ids, class_add_time_ids], dim=0)
        ...
        outputs = self.unet(
            ...
            mix_prompt_embeds,
            added_cond_kwargs={
                "time_ids": mix_add_time_ids,
                "text_embeds": mix_pooled_prompt_embeds,
            },
        ).sample
