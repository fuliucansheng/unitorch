...
class QWenImageText2ImageGeneration(GenericQWenImageModel):
    ...
    def generate(
        ...
        images = self.pipeline(
            ...
            negative_prompt_embeds=outputs.negative_prompt_embeds.to(torch.bfloat16),
            prompt_embeds_mask=outputs.prompt_embeds_mask.to(torch.bfloat16),
            negative_prompt_embeds_mask=outputs.negative_prompt_embeds_mask.to(
                torch.bfloat16
            ),
...
class QWenImageEditingGeneration(GenericQWenImageModel):
    ...
    def generate(
        ...
        images = self.pipeline(
            images=refer_vae_pixel_values.to(torch.bfloat16),
            prompt_embeds=outputs.prompt_embeds.to(torch.bfloat16),
            negative_prompt_embeds=outputs.negative_prompt_embeds.to(torch.bfloat16),
            prompt_embeds_mask=outputs.prompt_embeds_mask.to(torch.bfloat16),
            negative_prompt_embeds_mask=outputs.negative_prompt_embeds_mask.to(
