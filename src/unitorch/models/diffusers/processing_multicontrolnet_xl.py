...
class MultiControlNetXLProcessor:
    def __init__(
        self,
        vocab1_path: str,
        merge1_path: str,
    ...
    def text2image(
        ...
        condition_pixel_values = torch.Tensor(
            [
                self.condition_vision_processor(condition_image)
                for condition_image in condition_images
            ]
        )
    ...
    def text2image_inputs(
        ...
        for condition_image in condition_images:
            if isinstance(condition_image, str):
                condition_image = Image.open(condition_image)
            condition_image = condition_image.convert("RGB")
            process_condition_images.append(condition_image)

        condition_pixel_values = torch.stack(
            [
                self.vae_condition_image_processor.preprocess(condition_image)[0]
                for condition_image in process_condition_images
            ]
        )
