...
from unitorch.cli.pipelines.multicontrolnet_xl import (
    MultiControlNetXLForText2ImageGenerationPipeline,
    MultiControlNetXLForImage2ImageGenerationPipeline,
    MultiControlNetXLForImageInpaintingPipeline,
)
from unitorch.cli.webuis import matched_pretrained_names
from unitorch.cli.webuis import CannyWebUI, DPTWebUI


class MultiControlNetXLText2ImageWebUI(GenericWebUI):
    match_patterns = [
        "^stable-xl.*multicontrolnet",
    ]
    ...
    def start(self, pretrained_name, **kwargs):
        if self._status == "running":
            self.stop()
        self.config.set(
            "core/pipeline/multicontrolnet_xl/text2image",
            "pretrained_name",
            pretrained_name,
        )
        self._name = pretrained_name
        self._pipe = (
            MultiControlNetXLForText2ImageGenerationPipeline.from_core_configure(
                self.config
            )
        )
    ...
    def serve(
        ...
        conditioning_scale1: Optional[float] = 0.5,
        conditioning_scale2: Optional[float] = 0.5,
        conditioning_scale3: Optional[float] = 0.5,
        conditioning_scale4: Optional[float] = 0.5,
        conditioning_scale5: Optional[float] = 0.5,
...
@register_webui("core/webui/multicontrolnet_xl/image2image")
class MultiControlNetXLImage2ImageWebUI(GenericWebUI):
    match_patterns = [
        "^stable-xl.*multicontrolnet",
    ]
    pretrained_names = list(pretrained_diffusers_infos.keys())
    ...
    def start(self, pretrained_name, **kwargs):
        if self._status == "running":
            self.stop()
        self.config.set(
            "core/pipeline/multicontrolnet_xl/image2image",
            "pretrained_name",
            pretrained_name,
        )
        self._name = pretrained_name
        self._pipe = (
            MultiControlNetXLForImage2ImageGenerationPipeline.from_core_configure(
                self.config
            )
        )
        self._status = "running"
...
class MultiControlNetXLImageInpaintingWebUI(GenericWebUI):
    match_patterns = [
        "^stable-xl.*multicontrolnet",
    ]
    pretrained_names = list(pretrained_diffusers_infos.keys())
    ...
    def start(self, pretrained_name, **kwargs):
        if self._status == "running":
            self.stop()
        self.config.set(
            "core/pipeline/multicontrolnet_xl/inpainting",
            "pretrained_name",
            pretrained_name,
        )
        self._name = pretrained_name
        self._pipe = MultiControlNetXLForImageInpaintingPipeline.from_core_configure(
            self.config
        )
        self._status = "running"
        self._num_controlnets = len(
...
@register_webui("core/webui/multicontrolnet_xl")
class MultiControlNetXLWebUI(GenericWebUI):
    def __init__(self, config: CoreConfigureParser):
        webuis = [
            MultiControlNetXLText2ImageWebUI(config),
            MultiControlNetXLImage2ImageWebUI(config),
            MultiControlNetXLImageInpaintingWebUI(config),
            CannyWebUI(config),
            DPTWebUI(config),
        ]
