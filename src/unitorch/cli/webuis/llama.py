...
from unitorch.cli.pipelines.llama import LlamaForGenerationPipeline
...
class LlamaWebUI(SimpleWebUI):
    pretrained_names = list(pretrained_llama_infos.keys())
    supported_pretrained_names = matched_pretrained_names(
        pretrained_names,
        ["^llama-", "^llama2-", "^open-llama-", "^llama3-"],
    )
    pretrained_extension_names = list(pretrained_llama_extensions_infos.keys())
    supported_lora_names = matched_pretrained_names(
        pretrained_extension_names,
        ["^llama-lora-", "^llama2-lora-", "^open-llama-lora-", "^llama3-lora-"],
    )
