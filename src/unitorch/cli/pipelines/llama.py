...
class LlamaForGenerationPipeline(_LlamaForGeneration):
    ...
    def from_core_configure(
        cls,
        config,
        pretrained_name: Optional[str] = "llama-7b",
    ...
    def __call__(
        ...
        for ckpt, url, file, weight, alpha in zip(
            ...
            if ckpt is not None:
                lora_file = nested_dict_value(pretrained_llama_extensions_infos, ckpt)
...
@register_script("core/script/generation/llama")
class LlamaForGenerationScript(GenericScript):
    ...
    def launch(self, **kwargs):
        ...
        pipe = LlamaForGenerationPipeline.from_core_configure(config)

        config.set_default_section("core/script/generation/llama")
