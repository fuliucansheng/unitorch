...
class StableForImageInpaintingPipeline(GenericStableModel):
    ...
    def __call__(
        ...
        negative_prompt_embeds = self.text(
            ...
            # inputs.get("negative_attention_mask"),
        )[0]
...
@register_script("core/script/stable/inpainting")
class StableForImageInpaintingScript(GenericScript):
    ...
    def launch(self, **kwargs):
        ...
        pipe = StableForImageInpaintingPipeline.from_core_configure(config)

        config.set_default_section("core/script/stable/inpainting")
        ...
        mask_col = config.getoption("mask_col", None)
        ...
        assert mask_col in data.columns, f"Column {mask_col} not found in data."
        ...
        if neg_prompt_col in data.columns:
            for prompt, neg_prompt, image, mask in zip(
                data[prompt_col], data[neg_prompt_col], data[image_col], data[mask_col]
            ):
                image = Image.open(image)
                mask = Image.open(mask)
                result = pipe(prompt, image, mask, neg_text=neg_prompt)
                results.append(save(result))
        else:
            for prompt, image, mask in zip(
                data[prompt_col], data[image_col], data[mask_col]
            ):
                image = Image.open(image)
                mask = Image.open(mask)
                result = pipe(prompt, image, mask)
                results.append(save(result))
