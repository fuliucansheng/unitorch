...
class QWen3ForGenerationPipeline(_QWen3ForGeneration):
    ...
    @classmethod
    @add_default_section_for_init("core/pipeline/qwen3")
    ...
    @torch.no_grad()
    @add_default_section_for_function("core/pipeline/qwen3")
    def __call__(
        self,
        prompt: str,
        ...
        if use_chat_template:
            prompt = self.processor.chat_template(json.loads(prompt))
        ...
        for ckpt, url, file, weight, alpha in zip(
            ...
            if ckpt is not None:
                lora_file = nested_dict_value(
                    pretrained_qwen_extensions_infos, ckpt, "weight"
                )
