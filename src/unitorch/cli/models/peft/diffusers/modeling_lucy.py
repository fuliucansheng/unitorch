...
from unitorch.models.peft.diffusers import (
    LucyLoraForVideoEditingGeneration as _LucyLoraForVideoEditingGeneration,
)
...
from unitorch.cli.models.diffusers import load_weight, load_wan_text_weight
from unitorch.cli.models.diffusers.modeling_lucy import _lucy_model_kwargs
...
def _lucy_lora_model_kwargs(config, section: str):
...
@register_model(
    "core/model/diffusers/peft/lora/video_editing/lucy", diffusion_model_decorator
)
class LucyLoraForVideoEditingGeneration(_LucyLoraForVideoEditingGeneration):
    def __init__(
        self,
        config_path: str,
    ...
    @classmethod
    @config_defaults_init("core/model/diffusers/peft/lora/video_editing/lucy")
    def from_config(cls, config, **kwargs):
        section = "core/model/diffusers/peft/lora/video_editing/lucy"
        model_kwargs, replace_keys = _lucy_lora_model_kwargs(config, section)
        pretrained_infos = model_kwargs.pop("pretrained_infos")
        use_auth_token = model_kwargs.pop("use_auth_token")

        inst = cls(**model_kwargs)
        state_dict = _lucy_lora_state_dict(
            config,
            pretrained_infos,
            use_auth_token,
            replace_keys,
    ...
    @config_defaults_method("core/model/diffusers/peft/lora/video_editing/lucy")
    @autocast(
        device_type=("cuda" if torch.cuda.is_available() else "cpu"),
        dtype=(torch.bfloat16 if is_bfloat16_available() else torch.float32),
    )
