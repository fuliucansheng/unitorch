...
* `unitorch-launch` command provides a convenient way to execute a quick script or workflow using the unitorch package. It enables you to define and run lightweight operations that leverage the functionalities of the unitorch library.
...
* `unitorch-service` command is used to deploy a background non-model service, including hosting an HTTP server or exposing utility APIs. This command is designed for lightweight auxiliary services that do not require a heavy model backend.
